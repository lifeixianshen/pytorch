...
if gen_to_source := os.environ.get('GEN_TO_SOURCE'):
...
class FileManager(object):
    ...
    def write_outputs(self, filename):
        ...
        self._write_if_changed(
            filename, ''.join(f"{name};" for name in sorted(self.filenames))
        )
    ...
    def check_all_files_written(self):
        if len(self.undeclared_files) > 0:
            raise Exception(
                f"trying to write files {self.undeclared_files} which are not in the list of outputs this script produces. use will_write to add them."
            )
        if len(self.filenames) > 0:
            raise Exception(
                f"Outputs declared with 'will_write' were never written: {self.filenames}"
            )


TEMPLATE_PATH = f"{options.source_path}/templates"
GENERATOR_DERIVED = CodeTemplate.from_file(
    f"{TEMPLATE_PATH}/GeneratorDerived.h"
)
TYPE_DERIVED_CPP = CodeTemplate.from_file(f"{TEMPLATE_PATH}/TypeDerived.cpp")
SPARSE_TYPE_DERIVED_CPP = CodeTemplate.from_file(
    f"{TEMPLATE_PATH}/SparseTypeDerived.cpp"
)
TYPE_DERIVED_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/TypeDerived.h")
TYPE_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/Type.h")
TYPE_EXTENDED_INTERFACE_H = CodeTemplate.from_file(
    f"{TEMPLATE_PATH}/TypeExtendedInterface.h"
)
TYPE_DEFAULT_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/TypeDefault.h")
TYPE_DEFAULT_CPP = CodeTemplate.from_file(f"{TEMPLATE_PATH}/TypeDefault.cpp")

REGISTER_CPU_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/RegisterCPU.h")
REGISTER_CPU_CPP = CodeTemplate.from_file(f"{TEMPLATE_PATH}/RegisterCPU.cpp")

REGISTER_CUDA_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/RegisterCUDA.h")
REGISTER_CUDA_CPP = CodeTemplate.from_file(f"{TEMPLATE_PATH}/RegisterCUDA.cpp")

TENSOR_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/Tensor.h")
TENSOR_METHODS_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/TensorMethods.h")

FUNCTIONS_H = CodeTemplate.from_file(f"{TEMPLATE_PATH}/Functions.h")

NATIVE_FUNCTIONS_H = CodeTemplate.from_file(
    f"{TEMPLATE_PATH}/NativeFunctions.h"
)
...
def generate_storage_type_and_tensor(backend, density, scalar_type, declarations):
    ...
    env = {
        ...
        'isIntegralType': not is_floating_type,
    }
    if density == 'Dense':
        env['Tensor'] = f"{density_tag}{backend}{scalar_name}Tensor"
    env['Type'] = f"{density_tag}{backend}{scalar_name}Type"
    env['DenseTensor'] = f"{backend}{scalar_name}Tensor"
    ...
    env['TypeID'] = f'TypeID::{tag}'
    top_env['type_ids'].append(f'{tag},')
    ...
    if backend == 'CUDA':
        ...
        env['THType'] = f'Cuda{sname}'
        env['THStorage'] = f'THCuda{sname}Storage'
        env['THTensor'] = f'THCuda{sname}Tensor'
        ...
        env['THStorage'] = f"TH{scalar_name}Storage"
        env['THTensor'] = f'TH{scalar_name}Tensor'
    ...
    if env['DenseBackend'] == 'CPU':
        ...
        top_env['cpu_type_headers'].append(f"""#include "ATen/{env['Type']}.h\"""")
    else:
        assert env['DenseBackend'] == 'CUDA'
        top_env['cuda_type_registrations'].append(type_register)
        top_env['cuda_type_headers'].append(f"""#include "ATen/{env['Type']}.h\"""")
...
def declare_outputs():
    ...
    for backend, density, scalar_types in iterate_types():
        ...
        full_backend = f"Sparse{backend}" if density == "Sparse" else backend
        for kind in ["Type"]:
            ...
            if backend == 'CUDA':
                fm = cuda_file_manager
            fm.will_write(f"{full_backend}{scalar_name}{kind}.h")
            fm.will_write(f"{full_backend}{scalar_name}{kind}.cpp")
...
def filter_by_extension(files, *extensions):
    ...
    for file in files:
        filtered_files.extend(
            file for extension in extensions if file.endswith(extension)
        )
...
def generate_outputs():
    ...
    if errors:
        raise RuntimeError(
            f"Error while trying to compare source and generated files for {errors}. Source directory: {core_source_path}.  Generated directory: {core_install_dir}."
        )
    if mismatch:
        file_component = f"{','.join(mismatch)}"
        if len(mismatch) > 1:
            file_component = '{' + file_component + '}'
        update_cmd = f"cp {core_install_dir}/{file_component} {core_source_path}"
        raise RuntimeError(
            f'Source files: {mismatch} did not match generated files.  To update the source files, set environment variable GEN_TO_SOURCE or run \"{update_cmd}\"'
        )

declare_outputs()
if options.output_dependencies is not None:
    file_manager.write_outputs(options.output_dependencies)
    core_file_manager.write_outputs(f"{options.output_dependencies}-core")
    cuda_file_manager.write_outputs(f"{options.output_dependencies}-cuda")
else:
    generate_outputs()
